...
class MaterialPurchaseRequisition(models.Model):
    ...
    # my_list = fields.Many2many('res.users', compute='compute_my_list')

    # def compute_my_list(self):
    #     users_list = []
    #     print("Hello")
    #     locations = self.env['stock.location'].search([])
    #     for rec in locations:
    #         if rec.user_id:
    #             users_list.append(rec.user_id.id)
    #             # self.create({
    #             #     'my_list':rec.user_id.id
    #             # })
    ...
    state = fields.Selection([
        ...
        # ('approve', 'Approved'),
        ('stock', 'Internal Transfer Created'),
        ('receive', 'Received'),
        ('cancel', 'Cancelled'),
        ('reject', 'Rejected')],
        default='draft',
    ...
    #     location = self.env['stock.location'].search([('name', '=', 'Store')])
    #     return location
...
#                     raise Warning(_('Select Destination location under the picking details.'))
            ...
            rec.state = 'stock'
            if self.env.company.id == 44:
                if not rec.is_created_by_store:
                    stock_id.action_confirm()
                else:
                    stock_id.action_confirm()
                    stock_id.state = 'purchase_waiting'
                #else:
