...
class PayrollReportSelected(models.AbstractModel):
    _name = 'report.xlsx_payroll_report_selected.xlsx_payroll_report_new'
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        used_structures = []
        for sal_structure in lines.struct_id:
        ...
        rules = []
        col_no = 2
        for item in lines.struct_id.rule_ids:
    ...
    #         sheet.write(2, 0, 'Employee Name', format1)
    #         sheet.write(2, 1, 'Department', format1)
    #         sheet.write(2, 2, 'Period', format1)
    #         sheet.write(2, 3, 'Salary', format1)
    #         for rule in rules:
    #             sheet.write(2, rule[0] + 2, rule[2], format1)
    #
    ...
    #                     sheet.write(e_name, 0, slip.employee_id.name, format3)
    #                     sheet.write(e_name, 1, slip.employee_id.department_id.name, format3)
    #                     sheet.write(e_name, 2, str(slip.date_from.strftime('%B %d, %Y')) + '  To  ' + str(
    #                         slip.date_to.strftime('%B %d, %Y')), format3)
    #                     for line in slip.line_ids:
    #                         for rule in rules:
    #                             if line.code == rule[1]:
    #                                 if line.amount > 0:
